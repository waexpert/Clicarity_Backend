...
// Initialize the task service with database pool
const initializeTaskService = (databasePool) => {
  pool = databasePool;
  console.log('🔧 Task service initialized with database pool');
};
...
// Function to discover all "tasks" tables across all schemas
const discoverTaskTables = async () => {
  try {
    if (!pool) {
...
const generateSimplePdf = async (tasks) => {
  try {
    const employeeName = tasks[0].assigned_to;
    const filename = `${employeeName.replace(/\s+/g, '_')}_Tasks.pdf`;
    ...
    pdfDoc.fontSize(14)
      .font('Helvetica')
      .text(`Tasks for: ${employeeName}`, { align: 'left' });

    pdfDoc.moveDown(1);

    tasks.forEach((task, index) => {
      pdfDoc.fontSize(12)
        .text(`${index + 1}. ${task.task_name}`, { continued: false });
      
      pdfDoc.fontSize(10)
        .text(`Due Date: ${new Date(task.due_date).toLocaleDateString()}`, { indent: 20 });
      
      pdfDoc.fontSize(10)
        .text(`Notes: ${task.notes || 'None'}`, { indent: 20 });
    ...
    try {
      ...
      const url = await getSignedUrl(s3Client, getCommand, { expiresIn: 259200 });
      console.log(`Generated signed URL: ${url}`);
...
const generatePdfForTask = async (task) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM "${schema_name}"."${table_name}" WHERE assigned_to = $1`,
      [task.assigned_to]
    );
...
// Function to load all tasks due today from all discovered tables
const loadAllTasksDueToday = async () => {
  try {
    if (!pool) {
      throw new Error('Database pool not initialized');
    }
    ...
    for (const tableInfo of taskTables) {
      ...
      for (const task of tasks) {
        const enrichedTask = { ...task, schema_name, table_name };
        await queueTask(enrichedTask);
...
// Function to load tasks due today from default schema
const loadTasksDueToday = async () => {
  try {
    if (!pool) {
      throw new Error('Database pool not initialized');
...
// Start the task reminder system
const startTaskReminderSystem = async () => {
  try {
    console.log("📅 Task reminder system starting...");
    
    // Schedule task processing at 10:00 AM IST daily
    cron.schedule("05 0 * * *", loadAllTasksDueToday, {
      timezone: "Asia/Kolkata"
    });
    ...
    try {
      ...
      // Check if we need to run task processor immediately
      ...
      console.log("Task system continues without RabbitMQ. Will retry connection periodically.");
    }

    console.log("✅ Task reminder system started successfully");
  } catch (error) {
    console.error("❌ Failed to start task reminder system:", error);
  }
};

// Process tasks manually
const processTasks = async (allSchemas = true) => {
  try {
    if (allSchemas) {
      const tasksQueued = await loadAllTasksDueToday();
      return {
...
const handleGeneratePdf = async (req, res) => {
  try {
    ...
    const sanitizedQuery = `SELECT * FROM "${schemaName}"."${tableName}" WHERE assigned_to = $1;`;
    const result = await pool.query(sanitizedQuery, [assignedTo]);
...
// Download PDF endpoint handler
const handleDownloadPdf = async (req, res) => {
  try {
    const { url } = req.query;
